...
def mapped_boxplot(data, x_dim, y_dim, t_max, t_min=0, tau=1, linewidth=1, screen_size=24, screen_aspect=(16,10), screen_v_resolution=1200, max_v_screen_occupancy=0.8, compatible_output=True):
    """ 
    ...
    t_duration = t_max-t_min # compute the total time length to be plotted 

    # first part: massage the data into a convenient sorted form
    if compatible_output:
       sort_ids = numpy.lexsort((data[:,1], data[:,0]))  # sort the data by neuron first, then by time of spike.
       map_data = numpy.asarray(numpy.transpose([numpy.asarray(data[sort_ids,0], dtype=int)/y_dim, numpy.asarray(data[sort_ids,0], dtype=int)%y_dim, data[sort_ids,1]]), dtype=int)
       #map_data = numpy.array([[int(data[row, 0])/y_dim, int(data[row, 0])%y_dim, data[row, 1]] for row in sort_ids])
    else:
       sort_ids = numpy.lexsort((data[:,0], data[:,1]))  # sort the data by neuron first, then by time of spike.
       map_data = numpy.asarray(numpy.transpose([numpy.asarray(data[sort_ids,1], dtype=int)/y_dim, numpy.asarray(data[sort_ids,1], dtype=int)%y_dim, data[sort_ids,0]]), dtype=int)
    ...
    if (2*linewidth*t_duration*y_dim) > screen_v_resolution*max_v_screen_occupancy: 
       # adjust for thick linewidths that would occupy more than the entire screen when combined
       linewidth = math.floor(screen_v_resolution*max_v_screen_occupancy)/float(2*t_duration*y_dim)
       print "Warning: requested linewidth wider than maximum allowable."
       print "Setting linewidth to maximum possible: %f" % linewidth
    plot_size = [2*linewidth*t_duration*x_dim, 2*linewidth*t_duration*y_dim]  # total size of the topographic plot
    sizes = (2*linewidth*(t_max-map_data[:,2]+1))**2 # sizes of the possible boxes
...
def mapped_arrowplot(data, x_dim, y_dim, t_max, t_min=0, tau=1, arrowscale=1.0, linewidth=1, screen_size=24, screen_aspect=(16,10), screen_v_resolution=1200, max_v_screen_occupancy=1.0, compatible_output=True):
    """ 
    ...
    if compatible_output:
       sort_ids = [numpy.lexsort((data[dir_idx][:,1], data[dir_idx][:,0])) if len(data[dir_idx]) > 0 else numpy.array([]) for dir_idx in dir_idxs]
       map_data = [numpy.transpose([numpy.asarray(data[dir_idx][sort_ids[dir_idx],0],dtype=int)/y_dim, numpy.asarray(data[dir_idx][sort_ids[dir_idx],0],dtype=int)%y_dim, data[dir_idx][sort_ids[dir_idx],1]]) if len(sort_ids[dir_idx]) else numpy.array([]) for dir_idx in dir_idxs] 
       #map_data = [numpy.array([[int(data[dir_idx][row, 0])/y_dim, int(data[dir_idx][row, 0])%y_dim, data[dir_idx][row, 1]] for row in sort_ids[dir_idx][:]]) for dir_idx in dir_idxs]
    else:
       sort_ids = [numpy.lexsort((data[dir_idx][:,0], data[dir_idx][:,1])) if len(data[dir_idx]) > 0 else numpy.array([]) for dir_idx in dir_idxs]
       map_data = [numpy.transpose([numpy.asarray(data[dir_idx][sort_ids[dir_idx],1],dtype=int)/y_dim, numpy.asarray(data[dir_idx][sort_ids[dir_idx],1],dtype=int)%y_dim, data[dir_idx][sort_ids[dir_idx],0]]) for dir_idx in dir_idxs]  
    ...
    t_dim = int(math.ceil(math.sqrt(t_duration)))     # arrows per box-width
    ...
    # Next, compute the positions and angles of the arrows. Numpy can't flatten multi-dimensional
    # arrays with different element lengths on inner axes, thus the fromiter/chain methology. Angles
    # come from the direction positions are computed from the topographic position.
    angles_u = numpy.fromiter(itertools.chain.from_iterable([numpy.full(len(map_data[dir_idx]),math.cos((math.pi/num_dirs)*dir_idx)) for dir_idx in dir_idxs if len(map_data[dir_idx]) > 0]),dtype=float)
    angles_v = numpy.fromiter(itertools.chain.from_iterable([numpy.full(len(map_data[dir_idx]),math.sin((math.pi/num_dirs)*dir_idx)) for dir_idx in dir_idxs if len(map_data[dir_idx]) > 0]),dtype=float)
    ...
    positions_x = numpy.fromiter(itertools.chain.from_iterable([(0.5+(numpy.array(map_data[dir_idx][:,2]-t_min,dtype=int)/t_dim))+(map_data[dir_idx][:,0]*cell_dim) for dir_idx in dir_idxs if len(map_data[dir_idx]) > 0]),dtype=float)
    positions_y = numpy.fromiter(itertools.chain.from_iterable([(0.5+(numpy.array(map_data[dir_idx][:,2]-t_min,dtype=int)%t_dim))+(map_data[dir_idx][:,1]*cell_dim) for dir_idx in dir_idxs if len(map_data[dir_idx]) > 0]),dtype=float)
    ...
    dir_colours = numpy.fromiter(itertools.chain.from_iterable([dir_cmap for dir_cmap in dir_colours if len(dir_cmap) > 0]),dtype=int)
